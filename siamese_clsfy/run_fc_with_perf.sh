...
do
  echo $i
  #sudo /home/pi/perf/perf  stat -e $GENERAL,$BRANCH,$CACHE,$CAHCE_L1,$CACHE_LLC,$OS,$ARM -r $REPEAT -a ./perf_fc_layer.py $i l 2>&1 | tee stat_fc_$i 
  #sudo /home/pi/perf/perf  stat -e $GENERAL,$BRANCH,$CACHE,$CAHCE_L1,$CACHE_LLC,$OS,$ARM -r $REPEAT -a ./perf_fc_layer.py $i n 2>&1 | tee -a stat_fc_$i
  ./perf_fc_layer.py $i n 2>&1 | tee time_fc_$i
done
